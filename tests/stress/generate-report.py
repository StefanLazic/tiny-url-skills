...
Usage:
  ...
  The script produces:
    - tests/stress/output/requests_over_time.png
    - tests/stress/output/p95_latency_over_time.png
    - docs/stress-test-report-1000rps.md
...
def plot_requests_over_time(total_counts, create_counts, redirect_counts, output_path):
    ...
    # Target line at 10,000 per 10s bin (= 1000 RPS)
    ax.axhline(y=10000, color="red", linestyle=":", alpha=0.5, label="Target: 1000 RPS")

    ax.set_xlabel("Time (MM:SS)", fontsize=12)
    ax.set_ylabel("Number of Requests (per 10s bin)", fontsize=12)
    ax.set_title("Requests Executed Over Time — 1000 RPS Stress Test", fontsize=14)
...
def plot_p95_latency(create_p95, redirect_p95, output_path):
    ...
    ax.set_title("95th Percentile Latency Over Time — 1000 RPS Stress Test", fontsize=14)
    ax.legend(loc="upper left", fontsize=10)
    ax.grid(True, alpha=0.3)
...
def generate_markdown_report(stats, graphs_dir, output_path):
    ...
    report = f"""# Stress Test Report — 1000 RPS for 10 Minutes
...
![Requests Over Time]({rel_graphs}/requests_over_time.png)
...
![p95 Latency Over Time]({rel_graphs}/p95_latency_over_time.png)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python generate-report.py <k6-results.csv>")
        print("")
        print("First run the stress test with CSV output:")
        print("  k6 run --out csv=results.csv tests/stress/stress-test-1000rps.js")
        print("")
        print("Then generate the report:")
        print("  python tests/stress/generate-report.py results.csv")
        sys.exit(1)
    ...
    # Generate graphs
    requests_graph = os.path.join(OUTPUT_DIR, "requests_over_time.png")
    p95_graph = os.path.join(OUTPUT_DIR, "p95_latency_over_time.png")
    ...
    # Generate report
    report_path = os.path.join(REPO_ROOT, "docs", "stress-test-report-1000rps.md")
